...
为了更好的实现项目中类的管理，提供包的概念。`package` 将所有有关的类放在一个包类以便查找修改等，一些使用事项如下：
...
`import` 是引入包的关键字，它的使用场景如下：

1. 在源文件中使用 `import` 结构导入指定包下的类或者接口
...
例如：即使使用 `import XXX.*` 导入 `java.util` 包中的所有类，使用子包下的类仍需显式导入
...
```
5. 使用 `import static` 导入类的静态方法或静态变量
...
```
7. 如果使用的类或 `java.lang` 包下定义的，则可以省略 `import` 结构
...
```
8. 如果使用的类或接口是本包下定义的，则可以省略 `import` 结构
...
```
9. 当使用某个包的一些类时，仅需类名然后使用 ctrl+shift+o 或者选定类名（类或属性或方法）按住 ctrl+单击 即可自动插入类所在的包。

## 2. this

在Java中，`this` 的作用和其词义很接近，表示“当前”的意思。使用场景如下：

1. `this` 关键字可以用来修饰、调用：属性、方法、构造器。
2. `this` 关键字只能在方法内部使用。
3. 它在方法内部使用，即这个方法所属对象的引用。
    ...
    - `this` 引用的特性
        ...
        ![关键字2](/images/Grammer-DataStructure/java/关键字2.png)
4. 在类的方法中，我们可以使用 `this.属性` 或 `this.方法` 的方式，调用当前对象属性或方法。但是，通常情况下，我们都选择省略 `this.`。特殊情况下，如果方法的形参和类的属性同名时，我们必须显式的使用 `this.变量` 的方式，表明此变量是属性，而非形参。
    - 当我们实例化一个类时，可能会出现传入参数和这个类中的成员名相同，此时会引起歧义。
        ```java lineNumbers title="ExpClass.java"
        public class Date {
            public int year;
            public int month;
...
```
6. 在类的构造方法中，可以使用 `this(形参)` 的方式来调用本类其他构造方法。使用时必须声明在当前构造方法的首行，且只能有一个。
```java lineNumbers title="ExpClass.java"
public class demo3 {
    private int a;
    private int b;
...
`super` 理解为：父类的。下面是一些使用场景：

1. `super` 可用于访问父类中定义的属性和成员方法。
```java lineNumbers title="ExpClass.java"
class test{
    public int a = 20;
    public int b = 10;
...
```
3. 当子父类出现同名成员时，可以用 `super` 表明调用的是父类中的成员，`super` 的追溯不仅限于直接父类。
```java lineNumbers title="ExpClass.java"
class test{
    public int a = 10;
    public int b = 20;
...
```
4. `super` 和 `this` 的用法相像，`this` 代表本类对象的引用，`super` 代表父类的内存空间的标识。
